...
impl<V: Clone> EncodeIDMap<V> {
    ///EncodeIDMapに新規のEncodeIDと値を挿入する。
    /// 既存の範囲と重複がある場合は挿入時に調整が行われ、重複が排除される。
    pub fn insert(&mut self, encode_id: EncodeID, value: V) {
        //下位IDの個数が最小の次元を選択する
        ...
        if main_ancestors.is_empty() && min_count == 0 {
            self.uncheck_insert(encode_id, value);
            return;
        }
        ...
        for (f, x, y) in iproduct!(f_splited, x_splited, y_splited) {
            self.uncheck_insert(EncodeID { f, x, y }, value.clone());
        }
    }
}
