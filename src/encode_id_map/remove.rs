...
use crate::{
    ...
    encode_id::EncodeID,
    encode_id_map::{
        EncodeIDMap, Index, insert::split_self::RangesCollect,
        utils::select_dimensions::DimensionSelect,
    },
};

impl<V: Clone> EncodeIDMap<V> {
    ///EncodeIDMapから指定されたEncodeIDを削除する。
    /// 既存の範囲と重複がある場合は削除時に調整が行われ、削除されたID-値ペアが返される。
    pub fn remove(&mut self, encode_id: EncodeID) -> Vec<(EncodeID, V)> {
        let mut result = vec![];
        ...
        let mut need_insert: HashMap<EncodeID, V> = HashMap::new();
        ...
        for (i, (a_rel, b_rel)) in a_relation.0.iter().zip(b_relation.0.iter()).enumerate() {
            match (a_rel, b_rel) {
                ...
                (BitVecRelation::Ancestor, BitVecRelation::Ancestor) => {
                    ...
                    result.push((encode_id.clone(), main_ancestors_values[i].clone()));
                    ...
                    let value = main_ancestors_values[i].clone();
                    for (f, x, y) in iproduct!(f_splited, x_splited, y_splited) {
                        need_insert.insert(EncodeID { f, x, y }, value.clone());
                    }
                    ...
                    //分割後の範囲を挿入する
                    for (id, v) in need_insert {
                        self.uncheck_insert(id, v);
                    }
